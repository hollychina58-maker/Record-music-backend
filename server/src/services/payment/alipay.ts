...
export function createAlipayProvider(): PaymentProvider {
  return {
    ...
    async createPayment(input: CreatePaymentInput): Promise<PaymentResult> {
      ...
      // S5: stable out_trade_no — same order always maps to the same Alipay trade,
      // so re-opening the pay page can never overwrite payment_id (which would orphan
      // an already-paid QR code and leave the order stuck in pending).
      const outTradeNo = `ORDER_${input.orderId}`;
