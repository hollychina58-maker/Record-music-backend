...
// 3. Emotion → key + SPECIFIC BPM + rhythmic feel (a concrete tempo helps MiniMax
// match the rhythm far better than a wide range like "60-75 BPM").
const MOOD_MUSIC_PARAMS: Record<string, { keys: string[]; bpm: number; rhythm: string }> = {
  sorrow:      { keys: ['A minor', 'E minor', 'D minor'],    bpm: 66,  rhythm: 'slow 4/4, gentle rubato piano arpeggios' },
  joy:         { keys: ['C major', 'G major', 'D major'],    bpm: 120, rhythm: 'bright 4/4, driving upbeat groove' },
  peace:       { keys: ['D dorian', 'F major', 'C major'],   bpm: 60,  rhythm: 'floating 3/4, spacious and unhurried' },
  nostalgia:   { keys: ['G major', 'E minor', 'A minor'],    bpm: 82,  rhythm: 'moderate 4/4, gentle swaying folk pulse' },
  passion:     { keys: ['D minor', 'C minor', 'G minor'],    bpm: 128, rhythm: 'dramatic 4/4, powerful driving rhythm' },
  mystery:     { keys: ['E phrygian', 'B minor', 'F minor'], bpm: 66,  rhythm: 'slow 6/8, eerie and suspenseful' },
  warmth:      { keys: ['F major', 'C major', 'A major'],    bpm: 88,  rhythm: 'warm 4/4, tender romantic pulse' },
  loneliness:  { keys: ['A minor', 'E minor', 'D minor'],    bpm: 58,  rhythm: 'very slow 4/4, sparse and minimal' },
};
...
export async function generateMusic(text: string, options: MusicOptions = {}): Promise<{ audioUrl: string }> {
  ...
  // Prompt in English — MiniMax parses English style descriptors far better than mixed Chinese/English
  const moodName = profile.mood;
  // 3. Concrete tempo + rhythmic feel (rather than a BPM range) so the generated
  // rhythm actually matches the requested emotion.
  let prompt = [
    `${randomKey} key, ${musicParams.bpm} BPM, ${musicParams.rhythm}`,
    `${profile.style} style, ${moodName} mood`,
    `featuring ${profile.instruments}`,
    genreHint,
    isInstrumental ? 'instrumental, no vocals' : `song with ${moodName} vocals`,
    `about ${durationSec} seconds long`,
  ]
    .filter(Boolean)
    .join('. ');
  ...
  if (!isInstrumental) {
    if (options.lyricsMode !== 'story_as_lyrics') {
      ...
      prompt += `. Story theme: ${text.slice(0, 200)}`;
    } else {
  ...
  const MAX_RETRIES = 2; // Only retry once on ECONNRESET/5xx/429 — timeout fails fast
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    try {
      ...
      const code = err?.code;
      // 只重试明确的瞬时错误（网络抖动/5xx/429）。超时（ETIMEDOUT）不重试——
      // MiniMax 是同步长请求，超时意味着服务端可能还在生成，重试只会重复计费并
      // 制造重复任务，最终两个请求都超时导致误判 failed。
      const isTransient = (!status && code === 'ECONNRESET')
        || status >= 500
        || status === 429;
