...
router.get('/hero-image', asyncHandler(async (_req, res: Response) => {
...
router.post('/hero-image/generate', authMiddleware, adminMiddleware, asyncHandler(async (_req: AuthRequest, res: Response) => {
  try {
...
router.delete('/hero-image', authMiddleware, adminMiddleware, asyncHandler(async (_req: AuthRequest, res: Response) => {
  const row = await dbGet<{ value: string }>("SELECT value FROM site_config WHERE key = 'hero_image'");
  if (row?.value) {
